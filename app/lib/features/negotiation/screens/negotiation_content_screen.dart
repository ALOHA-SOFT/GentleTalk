  ...
  Widget _buildNegotiationCard(
    ...
    return GestureDetector(
      onTap: () async {
        ...
        int? owner = userNo is int ? userNo : int.tryParse(userNo?.toString() ?? '');
        ...
        if (opponent != null && currentUserNo == opponent) {
          if (trimmedStatus == '상대방대기') {
            Navigator.pushNamed(context, '/opponent-message-view',
                arguments: {'status': status, 'issueNo': issueNo});
          } else if (trimmedStatus == '중재안제시') {
            Navigator.pushNamed(context, '/opponent-final-proposal',
                arguments: {'status': status, 'issueNo': issueNo});
          } else {
            Navigator.pushNamed(context, '/negotiation-detail', arguments: {
              'status': status,
              'issueNo': issueNo,
              'isOpponentView': true,
            });
          ...
          children: [
            Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.folder_outlined, size: 30, color: Colors.white),
                ),
                ...
                const Icon(Icons.more_vert, size: 22, color: AppColors.textPrimary),
              ],
            ),
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 10.9),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 7.3, vertical: 9.08),
                  decoration: BoxDecoration(
                    color: progressColor,
                    borderRadius: BorderRadius.circular(90),
                  ),
...
class _NegotiationsHistoryTab extends StatefulWidget {
  ...
  @override
  State<_NegotiationsHistoryTab> createState() => _NegotiationsHistoryTabState();
}
  ...
  Widget _buildHistoryCard(
    ...
    return GestureDetector(
      onTap: () {
        ...
        if (currentUserNo != null && opponent != null && currentUserNo == opponent) {
          String routeName;
          if (trimmedStatus == '협상완료') {
            routeName = '/opponent-negotiation-success';
          } else if (trimmedStatus == '협상결렬') {
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      const Icon(Icons.star, size: 12, color: Color(0xFF8A6E00)),
